      ...
      <ul>
        <li v-for="(item, index) in goods" class="menu-item" :class="{'current': currentIndex === index}"
          ...
          <span class="text border-1px">
            <classMap v-show="item.type > 0" :item="item.type" class="icon"></classMap>{{item.name}}
          </span>
        </li>
      </ul>
    </div>
  ...
  export default {
    ...
    methods: {
      selectMenu(index, event) {
        ...
        let foodList = this.$refs.foodWrapper.getElementsByClassName('foodListHook');
